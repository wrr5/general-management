...
// 验证注册信息是否有效
func ValidatePhone(phone, name string) (bool, ApiResponse, error) {
	// 请求url
	...
	if apiResponse.DataObj.Total != 1 {
		return false, ApiResponse{}, fmt.Errorf("无效的手机号")
	}

	if len(apiResponse.DataObj.Records) == 0 {
		return false, ApiResponse{}, fmt.Errorf("响应数据中未找到记录")
	}
